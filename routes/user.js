  ...
  if (request.params.id) {
    user.getById(request.params.id, function(err, dbResult) {
      if (err) {
        ...
        response.json(dbResult); // 'Tulostaa' koko tietokannan tuloksen
        //response.json(dbResult[0].username); // 'Tulostaa' usernamen "teppo" - tyyliin
        //response.send(dbResult[0].username); // 'Tulostaa' usernamen ilman lainausmerkkejä, eli teppo
      }
    });
  } else {
